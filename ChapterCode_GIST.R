# Este codigo implementa todos los análisis reportados en:
# Baroja, J. L. (2015). Modelos Bayesianos Jerárquicos.

# Cuando el código se ejecuta de inicio a fin debe producir 9 archivos .pdf con todas las
# gráficas del capítulo en el directorio de trabajo actual.
# Aparte, todas las distribuciones posteriores de todos los modelos deben aparecer como 
# objetos en el espacio de trabajo de R.

# Para que el código funcione es necesario ejecutarlo en R:
# 1) R es un paquete de cómputo estadístico disponible en: [http://www.r-project.org/]  
# Aparte de R, es necesario instalar:
# 2) JAGS (Just Another Gibbs Sampler), disponible en: [http://sourceforge.net/projects/mcmc-jags/files/]
# 3) El paquete 'R2jags' en R. Para instalar 'R2jags' hay que ejecutar esta 
#     instrucción directamente en la consola de R (sin corchetes): [install.packages('R2jags')]

# Resumen:
# Registramos la cantidad de canciones que cada una de 15 personas bailó
# en cada una de 12 fiestas durante el semestre. 
# Con base en estas observaciones construimos 5 modelos (m0 - m4) para
# inferir el 'gusto por bailar' de cada individuo y de la población.

# m0: infiere el gusto por bailar de la Persona 14 (primer ejemplo del capítulo)
# m1: infiere el gusto por bailar de todas las personas sin suponer relaciones poblacionales entre ellas.
# m2: infiere el gusto por bailar de todas las personas y el de la población, 
#     asumiendo que las personas provienen de una población común respecto del gusto por bailar.
# m3: infiere el gusto por bailar de todas las personas y el de la población,
#     y también infiere el número de canciones que tocaron en cada fiesta, aunque no asume 
#     relaciones poblacionales entre fiestas.
# m4: infiere el gusto por bailar de todas las personas y el de la población,
#     y también infere el número de canciones que tocaron en cada fiesta asumiendo que éstas
#     provienen de una distribución poblacional común.
...
# Graficando datos 1
pdf_name <- 'data_1.pdf'
pdf(file=pdf_name,width=4,height=2.5)
layout(1)
...
pdf(file=pdf_name,width=5.5,height=7.5)
layout(1:3)
par(omi=c(0,0,0,0))
par(mai=c(0.5,0,0,0))
par(lend=1)
...
text(mean(nodo_2),max(density(nodo_2)$y+.14),
     ...
     col='black')
text(17.5,.5,'P4',cex=10,col='gray97')
legend(10,.5,yjust=0.5,xjust=0.5,
       legend=c(expression(paste('M'[1])),
